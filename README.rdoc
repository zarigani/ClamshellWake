...
===ターミナルからコマンドで操作する方法
* カーネルにロード用の<tt>ClamshellWake.kext</tt>を/tmpにコピーします。（カーネルは権限に厳格で、オーナー=root グループ=wheelしか受け付けません。）
  * <tt>sudo cp -R ClamshellWake.kext /tmp</tt>
  * <tt>sudo chown -R root:wheel /tmp/ClamshellWake.kext</tt>

* ClamshellWake.kextを有効にします。
  * <tt>sudo kextload -v /tmp/ClamshellWake.kext</tt>

* ClamshellWake.kextを無効にします。
  * <tt>sudo kextunload -v /tmp/ClamshellWake.kext</tt>

* アップル以外のカーネル機能拡張のロード状態を確認します。
  * <tt>kextstat | grep -v com.apple</tt>
