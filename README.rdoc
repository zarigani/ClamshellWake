...
==これは何？
+ClamshellWake.kext+（クラムシェル ウェイク）は、MacBookのディスプレイを閉じた状態でも {Wake on Demand}[http://support.apple.com/kb/HT3774?viewlocale=ja_JP] が有効になるカーネル機能拡張です。
...
==使い方
===簡単な方法
* 付属のClamshellWake.appを起動することでClamshellWake.kextが有効になります。
* 付属のClamshellWake.appを終了することでClamshellWake.kextも無効になります。

===ターミナルからコマンドで操作する方法
* カーネルにロード用のClamshellWake.kextを/tmpにコピーします。（カーネルは権限に厳格で、オーナー=root グループ=wheelしか受け付けません。）
<tt>sudo cp -R ClamshellWake.kext /tmp</tt>
<tt>sudo chown -R root:wheel /tmp/ClamshellWake.kext</tt>
